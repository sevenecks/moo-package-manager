...
Upgrading MPM is straight-forward. Just run `@load-package` and check for an available update in `Slither's MOO Packages`. If there is an update available, you can install it. The version you install with the install instructions above _may_ be out of date.
